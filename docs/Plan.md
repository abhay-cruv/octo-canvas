...
- **apps/bridge** — Python process baked into the Sprite image. Boots once when the sprite warms, dials home to the orchestrator over WSS, multiplexes N concurrent `claude` CLI subprocesses (one per active Task) driven by `claude-agent-sdk`, holds the in-process MCP server that registers `ask_user_clarification`, resolves Anthropic credentials via `ClaudeCredentials` (API key in v1; OAuth/BYOK reserved). Repos live in `/work/<full_name>/`; CLI session transcripts live in `~/.claude/projects/`.
...
- **Anthropic credentials**: never baked into the Sprite image. v1 passes `ANTHROPIC_API_KEY` via the sprite's env at provision time (it's a per-sprite secret); future credential modes (`user_oauth`, `user_api_key`) flow through the reserved `SessionEnv` WSS frame so user-scoped secrets never sit on disk in the sprite. See §14.7.
