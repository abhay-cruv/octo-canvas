...
7. **Pause is back — implemented via session-kill.** Sprites auto-pauses on idle, but for the case where a user wants to release compute *now* (cost), `POST /api/sandboxes/{id}/pause` lists active exec sessions and kills each via `POST /v1/sprites/{name}/exec/{session_id}/kill` (raw HTTP through the SDK's authenticated client — `kill_session` isn't in rc37 SDK methods). With no active sessions keeping it warm, Sprites' idle timer transitions the sprite to `cold` within seconds. Idempotent on `cold`. Slice 6+ will narrow the kill set to skip sessions tagged as agent runs so Pause doesn't murder an in-flight task. `POST /api/sandboxes/{id}/refresh` is also exposed for non-mutating status resyncs.
...
3. Click "Wake sandbox" → `POST /api/sandboxes/{id}/wake` calls `provider.spawn`. The panel transitions through `spawning` → `running`. The Fly dashboard shows a Sprite named `octo-sbx-<sandbox_id>` in the `iad` region with a 20 GB volume mounted at `/work`.
...
8. End-to-end manual run with **`SANDBOX_PROVIDER=fly`** + a real `SPRITES_API_KEY` (where available): same flow, the Fly dashboard shows `octo-sbx-<sandbox_id>` in `iad` with a 20 GB volume, transitioning through `started`/`stopped`/`destroyed`. After Reset, the Fly dashboard shows the *old Sprite is gone and a new one with the same name pattern but a new sprite id is up*; the old volume is gone, the new one is empty.
