...
These resolve the six open decisions from the slice-4 kickoff conversation plus two follow-up calls (#7, #8). Each is the recommended default; flip any of them by editing this brief while it's still active.

1. **Sprites SDK pinning** — pin `flyio-sprites>=0.X,<0.X+1` (exact version captured in `apps/orchestrator/pyproject.toml`) and treat the SDK as **wrapped behind `FlySpritesProvider`**. We never `import flyio_sprites` outside `python_packages/sandbox_provider/`. If the SDK doesn't exist as a pypi package yet, fall back to a thin REST client over `httpx` against the documented Sprites HTTP API; that's the path of least dependency churn.
2. **Default Fly region** — `iad` (Ashburn, VA). Stored in `Settings.fly_region` so prod can override per-deployment. No multi-region in v1; user's Sprite is wherever the orchestrator says.
3. **Naming-collision strategy** — **no salt suffix needed.** The Sprite name is `vibe-sbx-{sandbox_id}` where `sandbox_id` is the Mongo `Sandbox._id` (an ObjectId). On destroy we keep the Mongo doc with `status="destroyed"` (audit trail); the next "create" inserts a *new* Sandbox doc with a *new* ObjectId, so the Sprite name is unique by construction. **Reset rotates `sprite_id` on the same `Sandbox._id`**, so the Sprite name is reused — Fly must finish destroying the old Sprite before the new spawn (this is fine because reset is a sequential `provider.destroy` → `provider.spawn` chain, not parallel).
4. **`/work` disk cap** — **20 GB per Sprite, on a persistent volume.** The Sprite is spawned with a Fly volume mounted at `/work`. **The volume survives hibernate/wake.** It is destroyed by exactly two operations: `POST /api/sandboxes/{id}/reset` (and immediately replaced with a fresh empty volume) and `POST /api/sandboxes/{id}/destroy` (and not replaced). Eviction job (slice 5b territory, but the cap is set here) targets `node_modules`, `.venv`, `.git/objects` LRU when usage > 85%. Per-Sprite resource caps: 2 vCPU, 4 GB RAM, 20 GB volume. All three configurable via `Settings.sprite_cpu`, `sprite_ram_mb`, `sprite_disk_gb`.
5. **Redis schema** — three keys per active sandbox; Mongo is source of truth, Redis is hot cache only.
   - `sandbox:{sandbox_id}` — hash `{sprite_id, status, last_active_at}`. Refreshed on any state transition; **90s TTL** (extended on heartbeat in slice 5a).
   ...
   - `orchestrator_capacity:{instance_id}` — hash `{ws_connections, sandboxes_owned}`. **60s TTL**. For hot-shedding decisions in slice 5a; same rationale.
   - **No queue keys yet.** `sandbox:{user_id}:queue` and `sandbox:{user_id}:active_run` land in slice 6.
6. **Sandbox doc creation timing — lazy.** `POST /api/sandboxes` is idempotent: returns the user's existing singleton or creates one with `status="none"` if missing. `POST /api/sandboxes/{id}/wake` is what actually calls `provider.spawn`. No eager creation at signup — we don't want orphaned `Sandbox` docs for users who never get past `/dashboard`.
7. **Reset and Destroy are two distinct operations, not one button with a label tweak.** Reset wipes the volume + respawns a fresh Sprite for the same `Sandbox` doc (preserves `_id`, increments `reset_count`). Destroy fully tears down and marks the doc destroyed; the user must `Provision sandbox` to get a new one (with a new `_id`). Two separate API endpoints: `POST /api/sandboxes/{id}/reset` and `POST /api/sandboxes/{id}/destroy`. Two separate UI buttons with two separate confirmation dialogs. See §6 (routes), §10b (UI), §"State preservation contract" below.
8. **Provider selection is explicit, not silent.** New env var `SANDBOX_PROVIDER ∈ {"fly", "mock"}`, defaults to `fly`. With `fly`, an empty `SPRITES_API_KEY` aborts startup. With `mock`, the orchestrator boots and emits a `sandbox_provider.mock_in_use` warning every boot. **No silent fallback to mock when the API key is empty** — that path masks misconfigured prod deploys. CI must assert prod manifests don't carry `SANDBOX_PROVIDER=mock`. See §9.
