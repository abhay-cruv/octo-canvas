...
- **Status**: Slices 0–4 shipped. Slice 5a code shipped, awaiting sign-off — `/ws/web/tasks/{task_id}` (Pydantic discriminated unions in [`shared_models/wire_protocol/`](../python_packages/shared_models/src/shared_models/wire_protocol/)), atomic per-task `seq` via `seq_counters` raw collection, Mongo-replay → Redis pub/sub live mode via [`TaskFanout`](../apps/orchestrator/src/orchestrator/ws/task_fanout.py), 30/90s heartbeat, `Subscription`-tracked backpressure. Wire TS codegen: `pnpm --filter @octo-canvas/api-types gen:api-types` runs both `openapi-typescript` AND `gen:wire-types` (JSON Schema → `pnpm dlx json-schema-to-typescript`) — re-exported from `@octo-canvas/api-types`. Dev-only `/api/_internal/tasks*` endpoints gated by `ALLOW_INTERNAL_ENDPOINTS`. Next: slice 5b (clone + reconciliation + checkpoint-based reset).
