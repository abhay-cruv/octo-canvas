...
27. **Sprite image is a CI-built artifact** (slice 7). Bakes Node ≥ 20 + pinned `claude` CLI binary + bridge wheel + nvm/pyenv/rbenv + a static system-package baseline. Pin recorded in `apps/bridge/CLAUDE_CLI_VERSION`. Bridge boots with `claude --version` check and refuses to start on mismatch. Provider Protocol's `create()` accepts `image_tag` (slice 7); orchestrator's `BRIDGE_IMAGE_TAG` env defaults to `latest`.
