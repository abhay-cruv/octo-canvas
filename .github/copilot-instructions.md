...
- **Reuse before writing.** Search `apps/`, `packages/`, `python_packages/` for existing code before adding new code. Never duplicate.
...
- **Pydantic is wire-shape source of truth.** Never hand-edit `packages/api-types/generated/schema.d.ts`.
...
- **Do not introduce banned tools.** See [`AGENTS.md` §2.6](../AGENTS.md) for the full list (no Hono, Express, Next.js, Prisma, npm, yarn, etc.).
